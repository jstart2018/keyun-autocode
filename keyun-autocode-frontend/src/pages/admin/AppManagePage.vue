...
const editApp = (app: API.AppVO) => {
  // 检查权限：只有管理员、超级管理员或应用创建者可以编辑
  const loginUserStore = useLoginUserStore()
  const isAdmin = loginUserStore.loginUser.role === 'admin'
  const isSuperAdmin = loginUserStore.loginUser.role === 'super_admin'

  if (!isAdmin && !isSuperAdmin && app.userId !== loginUserStore.loginUser.id) {
